...
**
** $Id: output.c,v 1.1 2006-03-08 17:13:29 adcockj Exp $
**/
                         ...
                         float32_t **sample_buffer)
{
    ...
    case CONV(1,1):
        ...
        {
            real_t inp = input[hDecoder->internal_channel[0]][i];
            (*sample_buffer)[i] = (float)inp*FLOAT_SCALE;
        }
        break;
    case CONV(2,0):
        ch  = hDecoder->internal_channel[0];
        ...
        for(i = 0; i < frame_len; i++)
        {
            real_t inp0 = input[ch ][i];
            real_t inp1 = input[ch1][i];
            (*sample_buffer)[(i*2)+0] = (float)inp0*FLOAT_SCALE;
            (*sample_buffer)[(i*2)+1] = (float)inp1*FLOAT_SCALE;
        }
        break;
    default:
        for (ch = 0; ch < channels; ch++)
        {
            ...
            {
                real_t inp = get_sample(input, ch, i, hDecoder->downMatrix, hDecoder->internal_channel);
                (*sample_buffer)[(i*channels)+ch] = (float)inp*FLOAT_SCALE;
            }
        }
        break;
    }
