...
void url_write_packet(void *opaque, uint8_t *buf, int buf_size)
{
    URLContext *h = opaque;
    url_write(h, buf, buf_size);
}

int url_read_packet(void *opaque, uint8_t *buf, int buf_size)
{
    URLContext *h = opaque;
    return url_read(h, buf, buf_size);
}

int url_seek_packet(void *opaque, int64_t offset, int whence)
{
    URLContext *h = opaque;
...
static int dyn_buf_seek(void *opaque, offset_t offset, int whence)
{
    ...
    if (whence == SEEK_CUR)
        ...
        offset += d->size;
    if (offset < 0 || offset > int64_t_C(0x7fffffff))
        return -1;
    d->pos = offset;
    return 0;
}
