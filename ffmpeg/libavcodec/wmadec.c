...
typedef struct WMADecodeContext {
    ...
    __align16(float,exponents[MAX_CHANNELS][BLOCK_MAX_SIZE]);
    float max_exponent[MAX_CHANNELS];
    int16_t coefs1[MAX_CHANNELS][BLOCK_MAX_SIZE];
    __align16(float,coefs[MAX_CHANNELS][BLOCK_MAX_SIZE]);
    MDCTContext mdct_ctx[BLOCK_NB_SIZES];
    float *windows[BLOCK_NB_SIZES];
    __align16(FFTSample,mdct_tmp[BLOCK_MAX_SIZE]); /* temporary storage for imdct */
    /* output buffer for one frame and the last for IMDCT windowing */
    __align16(float,frame_out[MAX_CHANNELS][BLOCK_MAX_SIZE * 2]);
    /* last frame info */
...
static int wma_decode_init(AVCodecContext * avctx)
{
    ...
    if (s->use_noise_coding) {
...
#else
        {
            ...
            float norm;
            seed = 1;
            norm = (1.0 / (float)(int64_t_C(1) << 31)) * sqrt(3) * s->noise_mult;
            for(i=0;i<NOISE_TAB_SIZE;i++) {
                seed = seed * 314159 + 1;
...
// XXX: FIXME!! there's a bug somewhere which makes this mandatory under altivec
    ...
    for(ch = 0; ch < s->nb_channels; ch++) {
        if (s->channel_coded[ch]) {
             __align16(FFTSample,output[BLOCK_MAX_SIZE * 2]);
            float *ptr;
            int i, n4, index, n;
