...
void h263_encode_picture_header(MpegEncContext * s, int picture_number)
{
    ...
    if(s->h263_plus){
        for(i=0; i<2; i++){
            int div, error;
            div= (s->avctx->frame_rate_base*int64_t_C(1800000) + int64_t_C(500)*s->avctx->frame_rate) / ((int64_t_C(1000)+i)*s->avctx->frame_rate);
            div= clip(1, div, 127);
            error= ABS(s->avctx->frame_rate_base*int64_t_C(1800000) - (int64_t_C(1000)+i)*s->avctx->frame_rate*div);
            if(error < best_error){
                best_error= error;
                best_divisor= div;
                best_clock_code= i;
...
void h263_decode_done_vlc(MpegEncContext *s)
{
	EnterCriticalSection( &g_csStaticDataLock );
    ...
    if (--h263_vlc_done == 0) {
        ...
        free_rl(&rl_inter);
        free_rl(&rl_intra);
        free_rl(&rvlc_rl_inter);
        free_rl(&rvlc_rl_intra);
        free_rl(&rl_intra_aic);

        free_vlc_rl(&rl_inter);
        free_vlc_rl(&rl_intra);
        free_vlc_rl(&rvlc_rl_inter);
        free_vlc_rl(&rvlc_rl_intra);
        free_vlc_rl(&rl_intra_aic);
