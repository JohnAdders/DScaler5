...
__align8(uint16_t,inv_zigzag_direct16[64]) = {0, };
...
static int dct_sad8x8_c(/*MpegEncContext*/ void *c, uint8_t *src1, uint8_t *src2, int stride, int h){
    ...
    __align8(uint64_t,aligned_temp[sizeof(DCTELEM)*64/8]);
...
static int quant_psnr8x8_c(/*MpegEncContext*/ void *c, uint8_t *src1, uint8_t *src2, int stride, int h){
    MpegEncContext * const s= (MpegEncContext *)c;
    __align8 (uint64_t,aligned_temp[sizeof(DCTELEM)*64*2/8]);
    DCTELEM * const temp= (DCTELEM*)aligned_temp;
...
static int rd8x8_c(/*MpegEncContext*/ void *c, uint8_t *src1, uint8_t *src2, int stride, int h){
    ...
    __align8 (uint64_t,aligned_bak[stride]);
    #else
...
static void idct_ref_put(uint8_t *dest, int line_size, DCTELEM *block)
{
    idct_ref (block);
    put_pixels_clamped_c(block, dest, line_size);
}
static void idct_ref_add(uint8_t *dest, int line_size, DCTELEM *block)
{
    idct_ref (block);
    add_pixels_clamped_c(block, dest, line_size);
}
...
void dsputil_init(DSPContext* c, AVCodecContext *avctx)
{
    ...
    if(avctx->lowres==1){
        ...
        }else if(avctx->idct_algo==FF_IDCT_REFERENCE){
            c->idct_put= idct_ref_put;
            c->idct_add= idct_ref_add;
            c->idct    = idct_ref;
            c->idct_permutation_type= FF_NO_IDCT_PERM;
