...
int ff_h263_decode_end(AVCodecContext *avctx)
{
    ...
    if (s->h263_msmpeg4)
        ff_msmpeg4_decode_done(s);
    else
        h263_decode_done_vlc(s);
...
//printf("padding_bug_score: %d\n", s->padding_bug_score);
    ...
    s->avctx->xvid_build=s->xvid_build;
    s->avctx->ffmpeg_version=s->ffmpeg_version;
    s->avctx->lavc_build=s->lavc_build;
    s->avctx->divx_version=s->divx_version;
    s->avctx->divx_build=s->divx_build;
