...
typedef struct HYuvContext{
    ...
    int picture_number;
    int last_slice_end;
    __align8(uint8_t,temp[3][2560]);
    uint64_t stats[3][256];
    uint8_t len[3][256];
    uint32_t bits[3][256];
    VLC vlc[3];
    AVFrame picture;
    __align8(uint8_t,bitstream_buffer[1024*1024*3]); //FIXME dynamic alloc or some other solution
    DSPContext dsp; 
}HYuvContext;
...
static void generate_len_table(uint8_t *dst, uint64_t *stats, int size){
    ...
    int up[2*size];
#else
    uint64_t *counts=_alloca(sizeof(uint64_t)*2*size);
