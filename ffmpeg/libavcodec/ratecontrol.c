...
/*        "lastIQP",
    ...
    {
        RcOverride *rco= &s->avctx->rc_override[rce->rcOverrideIndex1-1];
    
        if(rco->qscale) 
            bits= qp2bits(rce, rco->qscale); //FIXME move at end to really force it?
        else
            bits*= rco->quality_factor;
    }
