...
*/
	...
	return -1;
    s->acmod = avctx->ac3mode;
    s->lfe = avctx->ac3lfe;
    s->nb_all_channels = channels;
    s->nb_channels = s->lfe?channels-1:channels; //channels > 5 ? 5 : channels;
    s->lfe_channel = s->lfe?channels-1:-1; //s->lfe ? 5 : -1;
                            ...
                            unsigned char *frame, int buf_size, void *data)
{
    ...
    for(ch=0;ch<s->nb_all_channels;ch++) {
        ...
        for(i=0;i<NB_BLOCKS;i++) {
            ...
            sptr = samples + (sinc * (N/2) * i) + avctx->ac3channels[ch];
            for(j=0;j<N/2;j++) {
                v = *sptr;
