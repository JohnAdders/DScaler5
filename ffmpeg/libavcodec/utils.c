...
void avcodec_get_context_defaults(AVCodecContext *s){
    ...
    s->sample_aspect_ratio.num=0;s->sample_aspect_ratio.den=1;// = (AVRational){0,1};
...
int av_reduce(int *dst_nom, int *dst_den, int64_t nom, int64_t den, int64_t max){
    ...
    den = ABS(den)/gcd;
    if(nom<=max && den<=max){
        a1.num=nom;a1.den=den;// = (AVRational){nom, den};
        den=0;
    }
    ...
    while(den){
        ...
        int64_t next_den= nom - den*x;
            ...
            a0= a1;
        a1.num=a2n;a1.den=a2d;//= (AVRational){a2n, a2d};
        nom= den;
        den= next_den;
        }
