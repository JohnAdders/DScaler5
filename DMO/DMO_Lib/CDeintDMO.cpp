////////////////////////////////////////////////////////////////////////////
// $Id: CDeintDMO.cpp,v 1.6 2003-05-21 17:05:57 adcockj Exp $
/////////////////////////////////////////////////////////////////////////////
...
void CDeintDMO::ProcessPlanarChroma704(BYTE* pInputData, BYTE* pOutputData, VIDEOINFOHEADER2* InputInfo, VIDEOINFOHEADER2* OutputInfo)
{
    // copy chroma over
    // the formats for YV12 and NV12 are different
    // but the both 
    ...
    {
        // copy V then U
        // there are 352 x biHeight/2 of V 
        // followed by 352 x biHeight/2 of U
        for(int i(0); i < InputInfo->bmiHeader.biHeight; ++i)
        {
            memset(pOutputData, 128, 4);
            memcpy(pOutputData + 4, pInputData, 352);
            memset(pOutputData + 352 + 4, 128, 4);
            pOutputData += OutputInfo->bmiHeader.biWidth / 2;
            pInputData += 352;
        }
    }
    ...
    {
        // copy U & V - there are 352 x biHeight/2 of UV samples
        for(int i(0); i < InputInfo->bmiHeader.biHeight / 2; ++i)
        {
            memset(pOutputData, 128, 8);
            memcpy(pOutputData + 8, pInputData, 704);
            memset(pOutputData + 704 + 8, 128, 8);
            pOutputData += OutputInfo->bmiHeader.biWidth;
            pInputData += 704;
        }
    }
...
void CDeintDMO::BlackYUY2For8(BYTE* pOutputData)
{
    *(pOutputData++) = 0;
    *(pOutputData++) = 128;
    *(pOutputData++) = 0;
